AdvancedTooltips = LibStub("AceAddon-3.0"):NewAddon("AdvancedTooltips",
                                                    "AceConsole-3.0",
                                                    "AceEvent-3.0",
                                                    "AceComm-3.0",
                                                    "AceTimer-3.0")
...
ReoriginationArray_Descriptions[0] =
    "Reorigination Array Hidden Quest completed for this week."
...
local configDefaults = {randomType = true}
...
local function GetCurrentColorAsHexDigits()
    return 255 * AdvancedTooltips_Config.R, 255 * AdvancedTooltips_Config.G,
           255 * AdvancedTooltips_Config.B
end

local function formatWithCurrentColor(string)
    return string.format(
               "|cff%02x%02x%02x" .. string.gsub(string, "%%", "%%%%") .. "|r",
               GetCurrentColorAsHexDigits())
end

local function colorPickerChanged(restore)
    if restore then
        AdvancedTooltips_Config.R, AdvancedTooltips_Config.G, AdvancedTooltips_Config.B =
            unpack(ColorPickerFrame.previousValues)
        return
    end
...
function SlashCmdList.AdvancedTooltips(msg)
    if msg == "" then
        ...
        print("        Stats - " .. statsString ..
                  " - Enable or disable the showing of stat -> percentage in tooltips. (on|off)")
        print(
            "        Color - Change the color of the text displayed in the tooltips. - " ..
                formatWithCurrentColor(string.format("Current Color",
                                                     GetCurrentColorAsHexDigits())))
    end
    ...
    if strings[1] == "color" or strings[1] == "Color" then
        ColorPickerFrame:SetColorRGB(AdvancedTooltips_Config.R,
                                     AdvancedTooltips_Config.G,
                                     AdvancedTooltips_Config.B)
        ColorPickerFrame.hasOpacity = false
        ColorPickerFrame.previousValues =
            {
                AdvancedTooltips_Config.R, AdvancedTooltips_Config.G,
                AdvancedTooltips_Config.B
            }
        ColorPickerFrame.func, ColorPickerFrame.cancelFunc = colorPickerChanged,
                                                             colorPickerChanged
...
function AdvancedTooltips:OnInitialize()
    if AdvancedTooltips_State == nil then AdvancedTooltips_State = {} end
...
local function itemEnchant(itemLink)
    local _, _, Color, Ltype, Id, Enchant, Gem1, Gem2, Gem3, Gem4, Suffix,
          Unique, LinkLvl, reforging, Name =
        string.find(itemLink,
                    "|?c?f?f?(%x*)|?H?([^:]*):?(%d+):?(%d*):?(%d*):?(%d*):?(%d*):?(%d*):?(%-?%d*):?(%-?%d*):?(%d*):?(%d*)|?h?%[?([^%[%]]*)%]?|?h?|?r?")
...
local function ProcessItem(itemLink, tooltip)
        ...
        for i = 1, 5, 1 do
            ...
            if item[i] ~= nil and item[i][E_CHANCE] ~= nil then
                ...
                if item[i][E_RPPM] == 1 then
                    ...
                    strRight = strRight ..
                                   string.format("%.2f%%", item[i][E_CHANCE])
                end
                tooltip:AddDoubleLine(strLeft, strRight,
                                      AdvancedTooltips_Config.R,
                                      AdvancedTooltips_Config.G,
                                      AdvancedTooltips_Config.B,
                                      AdvancedTooltips_Config.R,
                                      AdvancedTooltips_Config.G,
    ...
    if id == 158075 then
        ...
        if azeriteItemLocation ~= nil then
            local xp, totalLevelXP = C_AzeriteItem.GetAzeriteItemXPInfo(
                                         azeriteItemLocation)

            tooltip:AddDoubleLine("Rank", C_AzeriteItem.GetPowerLevel(
                                      azeriteItemLocation),
                                  AdvancedTooltips_Config.R,
                                  AdvancedTooltips_Config.G,
                                  AdvancedTooltips_Config.B,
                                  ...
                                  AdvancedTooltips_Config.G,
                                  AdvancedTooltips_Config.B)
            tooltip:AddDoubleLine("Progress", string.format("%d/%d (%.2f%%)",
                                                            xp, totalLevelXP,
                                                            xp / totalLevelXP *
                                                                100),
                                  AdvancedTooltips_Config.R,
...
local function alreadyAdded(str1, tooltip)
...
local function AddReoriginationInfo(tooltip)
    tooltip:AddLine(" ")
    -- Check the status of the reorigination count.
    ReoriginationLevel = 10
    while ReoriginationLevel > 0 and
        IsQuestFlaggedCompleted(ReoriginationArray_Stacks[ReoriginationLevel]) ==
        false do ReoriginationLevel = ReoriginationLevel - 1 end

    tooltip:AddDoubleLine("Reorigination Stacks: ", ReoriginationLevel .. "/10",
                          AdvancedTooltips_Config.R, AdvancedTooltips_Config.G,
                          AdvancedTooltips_Config.B, AdvancedTooltips_Config.R,
                          AdvancedTooltips_Config.G, AdvancedTooltips_Config.B)

    if ReoriginationLevel ~= 10 then
        local WeeklyKills = 3
        while WeeklyKills ~= 0 and
            IsQuestFlaggedCompleted(ReoriginationArray_Weekly[WeeklyKills]) ==
            false do WeeklyKills = WeeklyKills - 1 end
        ...
        tooltip:AddDoubleLine("Weekly: ", colorString .. WeeklyKills .. "/3",
                              AdvancedTooltips_Config.R,
                              AdvancedTooltips_Config.G,
                              AdvancedTooltips_Config.B,
                              AdvancedTooltips_Config.R,
        ...
        tooltip:AddDoubleLine("Weekly", "|cff00ff00Max",
                              AdvancedTooltips_Config.R,
                              AdvancedTooltips_Config.G,
                              AdvancedTooltips_Config.B,
                              AdvancedTooltips_Config.R,
    ...
    local stat_values = {}
    for i, v in pairs(stats) do stat_values[i] = GetCombatRating(i) end

    -- find the largest
    largest = 9
    for i, v in pairs(stat_values) do
        if stat_values[largest] < stat_values[i] then largest = i end
    end

    tooltip:AddDoubleLine("Bonus: ",
                          75 * ReoriginationLevel .. " " .. stats[largest],
                          AdvancedTooltips_Config.R, AdvancedTooltips_Config.G,
                          AdvancedTooltips_Config.B, AdvancedTooltips_Config.R,
                          AdvancedTooltips_Config.G, AdvancedTooltips_Config.B)
    ...
    if UldirVantus == true then
        ...
        tooltip:AddDoubleLine("Bonus (Vantus): ",
                              75 * ReoriginationLevel .. " " .. stats[largest],
                              AdvancedTooltips_Config.R,
                              AdvancedTooltips_Config.G,
                              AdvancedTooltips_Config.B,
...
local function GetSpellChanceInfo(rank)
    ...
    if data[E_CHANCE] ~= nil and data[E_CHANCE] < 100.0 then
        ...
        if data[E_RPPM] == 1 then
            ...
            local actualRPPM = data[E_CHANCE] *
                                   (1 + UnitSpellHaste("player") / 100)
    ...
    if data[E_ICD] ~= nil and data[E_ICD] ~= 0 then
        itemData["proc_icd"] = string.format("%.1f second ICD",
                                             data[E_ICD] / 1000)
    else
        itemData["proc_icd"] = ""
    end
...
local function AppendStatInfo(frame, frame_text, stat_value, stat_type,
                              stat_name, stat_string)

    -- Note, GetItemStats is NOT used here. This local function is useful,
    ...
    if stat_type == 26 then
        ...
        s = string.gsub(frame_text, stat_string .. " " .. stat_name,
                        string.format("%s %s " ..
                                          formatWithCurrentColor(
                                              string.format("(%.2f%%%%%%%%)",
                                                            mastery)),
                                      stat_string, stat_name))
        if (alreadyAdded(s)) then return end
        s2 = string.gsub(s, stat_name .. " by " .. stat_string,
                         string.format("%s by %s " ..
                                           formatWithCurrentColor(
                                               string.format("(%.2f%%%%%%%%)",
                                                             mastery)),
                                       stat_name, stat_string, mastery))
        frame:SetText(s2)
    else
        s = string.gsub(frame_text, stat_string .. " " .. stat_name,
                        string.format("%s %s " ..
...
local function updateStats(frame, text, arr)
...
local function scanStats(tooltip)
        ...
        if text then
            if IsComboStats(frame, text) == false then
                ...
                if sdata ~= nil then
                    sdata_raw = sdata
                    sdata = string.gsub(sdata, ",", "")
                    AppendStatInfo(frame, text, tonumber(sdata), 9,
                                   "critical strike rating", sdata_raw)
                    text = frame:GetText() -- refresh the text string
                end
...
local function ProcessOneOffs(rank, tooltip, full)

    local extendedAscendanceDescription =
        "The initial heal is divided evenly across all allies\nwithin 20 yards."
    ...
    if rank == 288749 and full == true then
        ...
        for i = 2, 30 do
            ...
            if frame then text = frame:GetText() end
            if text then
                local newtext = string.gsub(text, "all stats", "all stats " ..
                                                formatWithCurrentColor(
                                                    "(Primary Stat and Stamina)"))
        ...
        if (full) then
            tooltip:AddLine(" ")
            tooltip:AddLine(
                "Places buff with ID := 288784 on your character\nwhen Bwonsamdi is active near you.",
                unpack(GetColors()))
        end
    ...
    elseif rank == 277666 then
        ...
        for i = 2, 30 do
            ...
            if frame then text = frame:GetText() end
            if text then
                local newtext = string.gsub(text, "^Ancestral Resonance$",
                                            formatWithCurrentColor(
                                                "Ancestral Resonance (" ..
                                                    lustString .. ")"))
                frame:SetText(newtext)
        ...
        tooltip:AddDoubleLine("Ancestral Resonance (No " .. lustString .. ")",
                              "RPPM: 1.00 (" .. actualRPPMString .. ")",
                              AdvancedTooltips_Config.R,
                              AdvancedTooltips_Config.G,
                              AdvancedTooltips_Config.B,
                              AdvancedTooltips_Config.R,
                              AdvancedTooltips_Config.G,
                              AdvancedTooltips_Config.B)
        -- Gutripper, different PPM above 30% HP
    elseif rank == 266937 then

        -- Append (Below 30%) to the existing item already in the tooltip
        for i = 2, 30 do
            local frame = _G[tooltip:GetName() .. "TextLeft" .. i]
            local text
            if frame then text = frame:GetText() end
            if text then
                local newtext = string.gsub(text, "^" ..
                                                GetSpellChanceInfo(rank)["proc_name"] ..
                                                "$", formatWithCurrentColor(
                                                GetSpellChanceInfo(rank)["proc_name"] ..
                                                    " (Below 30%%)"))
                frame:SetText(newtext)
        ...
        local actualRPPM = 2.0 * (1 + UnitSpellHaste("player") / 100)
        local actualRPPMString = string.format("%.2f", actualRPPM)
        tooltip:AddDoubleLine(GetSpellChanceInfo(rank)["proc_name"] ..
                                  " (Above 30%)",
                              "RPPM: 2.00 (" .. actualRPPMString .. ")",
                              AdvancedTooltips_Config.R,
                              AdvancedTooltips_Config.G,
                              AdvancedTooltips_Config.B,
                              AdvancedTooltips_Config.R,
                              AdvancedTooltips_Config.G,
                              AdvancedTooltips_Config.B)
    elseif rank == 114052 and
        alreadyAdded(extendedAscendanceDescription, tooltip) == false then
        tooltip:AddLine(extendedAscendanceDescription,
                        AdvancedTooltips_Config.R, AdvancedTooltips_Config.G,
                        AdvancedTooltips_Config.B)
    elseif rank == 280021 and full then
        tooltip:AddLine(" ")
        tooltip:AddLine("Rate of healing is increased by haste.",
                        AdvancedTooltips_Config.R, AdvancedTooltips_Config.G,
                        AdvancedTooltips_Config.B)
    elseif rank == 292752 then
        ...
        for k, v in pairs(zandalariRacial) do
            tooltip:AddDoubleLine(k, v, AdvancedTooltips_Config.R,
                                  AdvancedTooltips_Config.G,
                                  AdvancedTooltips_Config.B,
                                  AdvancedTooltips_Config.R,
                                  AdvancedTooltips_Config.G,
        ...
        -- Mag'har Orc racial
    elseif rank == 274738 then
        tooltip:AddLine(" ")
        tooltip:AddLine(
            "Grants 411 (at level 120) of a random secondary stat for\n15 seconds.",
            AdvancedTooltips_Config.R, AdvancedTooltips_Config.G,
            AdvancedTooltips_Config.B)
        tooltip:AddLine(" ")
        ...
        for k, v in pairs(stats) do
            local text = ""
            rating_coef = stat_array[k][120]

            if k == 26 then
                -- Mastery, special
                mastery = 411 / rating_coef
                mastery = mastery * select(2, GetMasteryEffect())
                text = string.format("%.2f%%", mastery)
            else
                text = string.format("%.2f%%", 411 / rating_coef)
            end

            tooltip:AddDoubleLine("411 " .. stats[k], text,
                                  AdvancedTooltips_Config.R,
                                  AdvancedTooltips_Config.G,
                                  AdvancedTooltips_Config.B,
                                  AdvancedTooltips_Config.R,
...
local function SpellTooltip(rank, tooltip, statsAdded)
    str = ""
    str2 = ""
    Header = false
    if AdvancedTooltips.SpellData[rank] ~= nil then
        ...
        if str2 ~= "" then
            tooltip:AddDoubleLine(str, str2, AdvancedTooltips_Config.R,
                                  AdvancedTooltips_Config.G,
                                  AdvancedTooltips_Config.B,
                                  AdvancedTooltips_Config.R,
                                  AdvancedTooltips_Config.G,
        ...
        if str3 ~= "" then
            tooltip:AddDoubleLine(" ", str3, AdvancedTooltips_Config.R,
                                  AdvancedTooltips_Config.G,
                                  AdvancedTooltips_Config.B,
                                  AdvancedTooltips_Config.R,
                                  AdvancedTooltips_Config.G,
    ...
    if rank == 280555 or rank == 280559 then AddReoriginationInfo(tooltip) end

    -- Try collecting stats
    if (statsAdded == false) then scanStats(tooltip) end
...
local function GetAzeriteSpellID(powerID)
...
local function ScanForTrait(tooltip, powerName)
    ...
    local atooltipPattern =
        "||cFF00FF00%s-||T%d-:20:20:0:0:64:64:4:60:4:60||t%s-" .. powerName ..
            "||r"
...
local function AddEnchantInfo(tooltip, itemHeaderAdded, spellID)
    spellData = GetSpellChanceInfo(spellID)
    if spellData ~= nil then
        if itemHeaderAdded == false then tooltip:AddLine(" ") end
        -- Remove (DND) from some enchant strings.
        tooltip:AddDoubleLine(gsub(spellData["proc_name"], "%(DND%)", ""),
                              spellData["proc_info"], AdvancedTooltips_Config.R,
                              AdvancedTooltips_Config.G,
                              AdvancedTooltips_Config.B,
                              AdvancedTooltips_Config.R,
                              AdvancedTooltips_Config.G,
        ...
        if spellData["proc_icd"] ~= "" then
            tooltip:AddDoubleLine(" ", spellData["proc_icd"],
                                  AdvancedTooltips_Config.R,
                                  AdvancedTooltips_Config.G,
                                  AdvancedTooltips_Config.B,
                                  AdvancedTooltips_Config.R,
...
local function ProcessItemOneOffs(id, header, tooltip)

    -- Ward of Envelopment
    -- Increased by 7.5% per ally, up to 30%
    if id == 165569 then

        if header == false then tooltip:AddLine(" ") end

        tooltip:AddLine("Absorption increased by 7.5% per ally, up to 30%.",
                        AdvancedTooltips_Config.R, AdvancedTooltips_Config.G,
                        AdvancedTooltips_Config.B)
    elseif id == 165928 or id == 165928 or id == 165926 or id == 152632 then
        tooltip:AddLine(" ")
        tooltip:AddDoubleLine("Potion of Replenishment", "35,084",
                              AdvancedTooltips_Config.R,
                              AdvancedTooltips_Config.G,
                              AdvancedTooltips_Config.B,
                              AdvancedTooltips_Config.R,
                              AdvancedTooltips_Config.G,
                              AdvancedTooltips_Config.B)
        tooltip:AddDoubleLine("Coastal Mana Potion", "15,557",
                              AdvancedTooltips_Config.R,
                              AdvancedTooltips_Config.G,
                              AdvancedTooltips_Config.B,
                              AdvancedTooltips_Config.R,
                              AdvancedTooltips_Config.G,
                              AdvancedTooltips_Config.B)
        tooltip:AddDoubleLine("Coastal Rejuvenation Potion", "11,668",
                              AdvancedTooltips_Config.R,
                              AdvancedTooltips_Config.G,
                              AdvancedTooltips_Config.B,
                              AdvancedTooltips_Config.R,
    ...
    elseif id == 165567 or id == 165512 then
        tooltip:AddLine(" ")
        tooltip:AddLine(
            "The following instances count as in Zuldazar:\nBattle for Dazar'alor, King's Rest, Atal'Dazar",
            AdvancedTooltips_Config.R, AdvancedTooltips_Config.G,
            AdvancedTooltips_Config.B)
    end
...
local function OnTooltip_Item(self, tooltip)
    ...
    if C_AzeriteEmpoweredItem.IsAzeriteEmpoweredItemByID(linkToID(link)) then
        local tierInfo = C_AzeriteEmpoweredItem.GetAllTierInfoByItemID(link,
                                                                       select(3,
                                                                              UnitClass(
                                                                                  "player")))
        for i = 1, 4, 1 do
            if tierInfo[i] ~= nil then
                for k, v in pairs(tierInfo[i]["azeritePowerIDs"]) do
                    if ScanForTrait(self, select(1, GetSpellInfo(
                                                     GetAzeriteSpellID(v)))) then
                        local spellInfo =
                            GetSpellChanceInfo(GetAzeriteSpellID(v))
                        if spellInfo ~= nil then
                            -- If we have strings
                            if spellInfo["proc_name"] ~= nil and
                                string.len(spellInfo["proc_name"]) > 0 then
                                if itemHeaderAdded == false then
                                    tooltip:AddLine(" ")
                                    itemHeaderAdded = true
                                end
                                tooltip:AddDoubleLine(spellInfo["proc_name"],
                                                      spellInfo["proc_info"],
                                                      AdvancedTooltips_Config.R,
                                                      AdvancedTooltips_Config.G,
                                                      AdvancedTooltips_Config.B,
                                                      ...
                                                      AdvancedTooltips_Config.G,
                                                      AdvancedTooltips_Config.B)
                                ProcessOneOffs(GetAzeriteSpellID(v), tooltip,
                                               false)
                            end
                        end
                    end
    ...
    if itemEnchant(link) ~= nil and itemEnchant(link) ~= 0 then
        if AdvancedTooltips.EnchantData[itemEnchant(link)] ~= nil then
            AddEnchantInfo(tooltip, itemHeaderAdded,
                           AdvancedTooltips.EnchantData[itemEnchant(link)])
        end
    end
    ...
    if AdvancedTooltips.BackupData[linkToID(link)] ~= nil then
        AddEnchantInfo(tooltip, itemHeaderAdded,
                       AdvancedTooltips.EnchantData[AdvancedTooltips.BackupData[linkToID(
                           link)]])
    end
...
local function dbg_PrintNotNil(s1, s2)
    if s2 ~= nil then
        print(s1 .. ": " .. s2)
    else
        print(s1 .. ": nil")
...
local function CheckIsAzeriteEssenceAndProcess(self, tooltip)
    local statsAdded = false

    -- Check if we're an azerite essence tooltip

    for i = 2, 60 do
        -- Obtain the text left
        local frame = _G[tooltip:GetName() .. "TextLeft" .. i]
        if frame ~= nil then
            local text = frame:GetText()
            if text ~= nil and string.match(text, "|cff71d5ffMajor Power:|r") then
                -- Our next line is the major power name
                local frame2 = _G[tooltip:GetName() .. "TextLeft" .. i + 1]
                local powerName = frame2:GetText()
                if powerName ~= nil then
                    SpellTooltip(AdvancedTooltips.azItems[powerName].Major,
                                 tooltip, statsAdded)
                    statsAdded = true
                end
            end

            if text ~= nil and string.match(text, "|cff8788eeMinor Power:|r") then
                -- Our next line is the major power name
                local frame2 = _G[tooltip:GetName() .. "TextLeft" .. i + 1]
                local powerName = frame2:GetText()
                if powerName ~= nil then
					if AdvancedTooltips.azItems[powerName] ~= nil then
                        SpellTooltip(AdvancedTooltips.azItems[powerName].Minor,
                                     tooltip, statsAdded)
                        statsAdded = true
                    end
                end
...
local function OnTooltipSpell(self, tooltip)
    ...
    if rank ~= nil then
        SpellTooltip(rank, tooltip, false)
    else
        CheckIsAzeriteEssenceAndProcess(self, tooltip)
    end
    tooltip:Show()
end
...
function AdvancedTooltips:OnEnable()
    ...
    GameTooltip:HookScript("OnTooltipSetItem",
                           function(...) OnTooltip_Item(..., GameTooltip) end)
    ItemRefTooltip:HookScript("OnTooltipSetItem", function(...)
        OnTooltip_Item(..., ItemRefTooltip)
    end)
    ShoppingTooltip1:HookScript("OnTooltipSetItem", function(...)
        OnTooltip_Item(..., ShoppingTooltip1)
    end)
    ShoppingTooltip2:HookScript("OnTooltipSetItem", function(...)
        OnTooltip_Item(..., ShoppingTooltip2)
    end)
    GameTooltip:HookScript("OnTooltipSetSpell",
                           function(...) OnTooltipSpell(..., GameTooltip) end)
    ItemRefTooltip:HookScript("OnTooltipSetSpell", function(...)
        OnTooltipSpell(..., ItemRefTooltip)
    end)
    -- WorldMapTooltip.ItemTooltip.Tooltip:HookScript('OnTooltipSetItem', local function(...) OnTooltip_Item(..., WorldMapTooltip.ItemTooltip.Tooltip) end)
end

function AdvancedTooltips:AfterEnable() end
